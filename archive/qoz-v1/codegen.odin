...
codegen :: proc(asts: map[string]^Node, sorted_packages: []string, ctx_sem: ^Semantic_Context, allocator := context.allocator) -> (res: string, err: mem.Allocator_Error) {
    ...
    // Pass 4.5: Generate array wrapper bodies for struct-based arrays
    // (Structs are now fully defined, so we can generate wrappers that reference them)
    for pkg_dir in sorted_packages {
        pkg_base := sanitize_c_identifier(filepath.base(pkg_dir), allocator)
        ctx_cg.current_pkg_name = pkg_base
        pkg_files := get_sorted_package_files(asts, pkg_dir, allocator)
        for file_path in pkg_files {
            codegen_scan_struct_fields_for_arrays(&ctx_cg, asts[file_path])
        }
    }
    ...
    // Pass 7: Generate array wrapper bodies for function return types
...
struct_has_vec_fields :: proc(ctx_cg: ^Codegen_Context, struct_type: Struct_Type, pkg_name: string) -> bool {
    for field in struct_type.fields {
        ...
        if _, is_vec := resolved_field_type.(Vec_Type); is_vec {
            ...
            if _, is_vec := ptr_type.pointee^.(Vec_Type); is_vec {
                return true
            }
        }
    }
...
struct_has_struct_array_fields :: proc(ctx_cg: ^Codegen_Context, struct_type: Struct_Type, pkg_name: string) -> bool {
    for field in struct_type.fields {
        resolved_field_type := field.type
        if named_type, is_named := field.type.(Named_Type); is_named {
            if resolved, ok := resolve_named_type(ctx_cg.ctx_sem, named_type, pkg_name); ok {
        ...
        if arr_type, is_array := resolved_field_type.(Array_Type); is_array {
            // Check if element type is a struct (not primitive)
            if type_contains_struct(arr_type.element_type^) {
                return true
            }
        }
    }
...
// Scan struct definitions for struct-based array fields and generate array wrappers
codegen_scan_struct_fields_for_arrays :: proc(ctx_cg: ^Codegen_Context, node: ^Node) {
    #partial switch node.node_kind {
    case .Program:
        for stmt in node.payload.(Node_Statement_List).nodes {
            codegen_scan_struct_fields_for_arrays(ctx_cg, stmt)
        }
    case .Var_Def:
        var_def := node.payload.(Node_Var_Def)
        ...
        if var_def.content.node_kind == .Type_Expr {
            ...
            if struct_type, is_struct := type_expr.type_info.(Struct_Type); is_struct {
                // Scan all fields for struct-based array types
                for field in struct_type.fields {
                    // Resolve the field type to handle type aliases
                    resolved_field_type := field.type
                    if named_type, is_named := field.type.(Named_Type); is_named {
                    ...
                    // Check if it's an array type with struct elements
                    if arr_type, is_array := resolved_field_type.(Array_Type); is_array {
                        if type_contains_struct(arr_type.element_type^) {
                            codegen_array_wrapper_struct(ctx_cg, arr_type, only_primitives = false)
                        }
                    }
                }
            } else if union_type, is_union := type_expr.type_info.(Union_Type); is_union {
                // Scan union variants for struct-based array fields
                for variant_type in union_type.types {
                    ...
                    // Check if variant is a struct with struct-based array fields
                    if struct_type, is_struct := resolved_variant.(Struct_Type); is_struct {
                        for field in struct_type.fields {
                            resolved_field_type := field.type
                            if named_type, is_named := field.type.(Named_Type); is_named {
            ...
            if struct_type, is_struct := type_expr.type_info.(Struct_Type); is_struct {
                if len(var_def.names) > 0 {
                    if !struct_has_vec_fields(ctx_cg, struct_type, ctx_cg.current_pkg_name) && 
                       !struct_has_struct_array_fields(ctx_cg, struct_type, ctx_cg.current_pkg_name) {
                        codegen_composite_definition(ctx_cg, type_expr.type_info, var_def.names[0])
                    }
                }
